...
class StabilityMargins:
    ...
    def __str__(self):
        ...
        gain_margin_db = f"{self.gain_margin_db:.2f}" if self.gain_margin_db is not None else "N/A"
        phase_margin = f"{self.phase_margin:.2f}" if self.phase_margin is not None else "N/A"
        gain_crossover_freq = f"{self.gain_crossover_freq:.3f}" if self.gain_crossover_freq is not None else "N/A"
        phase_crossover_freq = f"{self.phase_crossover_freq:.3f}" if self.phase_crossover_freq is not None else "N/A"
        if self.is_stable is None:
            stability_str = "N/A"
        else:
            stability_str = 'ESTÁVEL' if self.is_stable else 'INSTÁVEL'
        result += f"Margem de Ganho: {gain_margin_db} dB\n"
        result += f"Margem de Fase: {phase_margin}°\n"
        result += f"Freq. Cruzamento de Ganho: {gain_crossover_freq} rad/s\n"
        result += f"Freq. Cruzamento de Fase: {phase_crossover_freq} rad/s\n"
        result += f"Estabilidade: {stability_str}\n"
        return result
