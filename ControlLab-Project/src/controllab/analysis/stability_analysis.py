...
class StabilityAnalysisEngine:
    ...
    def __init__(self):
        ...
        self.frequency_analyzer = FrequencyAnalyzer() if FrequencyAnalyzer else None
    ...
    def comprehensive_analysis(self, tf_obj, show_all_steps: bool = True,
        ...
        if self.root_locus_analyzer:
            try:
                locus_features = self.root_locus_analyzer.analyze_comprehensive(tf_obj, show_steps=show_all_steps)
                report.root_locus_results = locus_features
                
                # Determinar estabilidade baseada no lugar geométrico
                if hasattr(locus_features, 'stability_assessment'):
                    stability_info = locus_features.stability_assessment
                    conclusion = f"K estável em {stability_info.get('stable_range', 'range indefinido')}"
                else:
        ...
        if self.frequency_analyzer:
            try:
                margins = self.frequency_analyzer.calculate_gain_phase_margins(tf_obj, show_steps=show_all_steps)
                report.frequency_response_results = margins
