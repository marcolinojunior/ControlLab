...
def calculate_performance_metrics(
    ...
    try:
        ...
        if poles:
            # Polo dominante (menor parte real negativa)
            dominant_poles = []
            for pole in poles:
                if pole.is_real:
                    if pole < 0:  # Polo real estável
                        dominant_poles.append(complex(float(pole), 0))
                else:
                    # Polo complexo
                    real_part = sp.re(pole)
                    imag_part = sp.im(pole)
                    if real_part < 0:  # Polo estável
                        dominant_poles.append(complex(float(real_part), float(imag_part)))
